...
export interface WorkoutItem {
  ...
  /**
   * Явно пропущено в режиме «идёт» (само или через подтверждение при
   * завершении). Компактная карточка показывает такое перечёркнутым.
   * Отметка «выполнено» снимает пропуск. Отсутствует = не пропускала.
   */
  skipped?: boolean;
  /** Сколько подходов отмечено кружками в режиме «идёт» (0..план) */
  setsDone?: number;
...
export interface Workout {
  ...
  /**
   * Режим «идёт»: ISO-время нажатия «Начать тренировку». status при этом
   * остаётся 'planned', поэтому старые читатели ничего не замечают.
   * После завершения поле сохраняется (длительность тренировки).
   */
