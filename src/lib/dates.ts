...
/** "13 июня 2025" — без хвостового « г.», одинаково на всех экранах */
export function fmtDate(iso: string): string {
  return toDate(iso)
    .toLocaleDateString(RU, { ...UTC, day: 'numeric', month: 'long', year: 'numeric' })
    .replace(/\s*г\.$/, '');
}

/** "13 июня" */
export function fmtDateShort(iso: string): string {
  return toDate(iso).toLocaleDateString(RU, { ...UTC, day: 'numeric', month: 'long' });
}

/** "пятница" */
export function fmtWeekday(iso: string): string {
  return toDate(iso).toLocaleDateString(RU, { ...UTC, weekday: 'long' });
}
...
/** "Июнь 2025" (по ключу месяца или полной дате) */
export function fmtMonthYear(isoOrKey: string): string {
  const iso = isoOrKey.length === 7 ? isoOrKey + '-01' : isoOrKey;
  const s = toDate(iso).toLocaleDateString(RU, { ...UTC, month: 'long', year: 'numeric' });
