...
/**
 * «12+12», «4+4 кг» — пара одинаковых гантелей: за подход двигаются ОБЕ,
 * т.е. общая нагрузка вдвое больше value (там вес одной гантели).
 * Неравные пары («4+6») парой не считаем — value у них и так null.
 */
export function isPairWeight(raw: string | null | undefined): boolean {
  if (!raw) return false;
  const m = /^(\d+(?:[.,]\d+)?)\s*\+\s*(\d+(?:[.,]\d+)?)\s*(?:кг\.?)?$/i.exec(raw.trim());
